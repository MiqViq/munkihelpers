...
# create_munki_single_item_uninstall_manifests

# create single item uninstall manifests from all Munki items that are not update_for

echo "CREATING SINGLE-ITEM UNINSTALL MANIFESTS FOR MUNKI..."

munkiRepoDir="/path/to/munki/repo"

scanDir="$munkiRepoDir/pkgsinfo"

dirToCreateManifests="$HOME/Desktop/munki_single_item_uninstall_manifests"
...
do
	...
	if ! defaults read "$dirToCreateManifests/$munkiItem" managed_uninstalls 2>/dev/null | grep -q -w -o "$munkiItem"
	then
		defaults write "$dirToCreateManifests/$munkiItem" managed_uninstalls -array-add "$munkiItem"
	fi
